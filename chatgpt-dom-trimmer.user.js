...
// @downloadURL  https://raw.githubusercontent.com/721676/tampermonkey_scripts/main/chatgpt-dom-trimmer.user.js
// @updateURL    https://raw.githubusercontent.com/721676/tampermonkey_scripts/main/chatgpt-dom-trimmer.meta.js
// @homepageURL  https://github.com/721676/tampermonkey_scripts
// @supportURL   https://github.com/721676/tampermonkey_scripts/issues
...
(function () {
  ...
  const EXCLUDED_PATHS = ["/gpts/editor", "/codex"];
  ...
  const STORAGE_KEY = "gptDomTrimSettings_v4";
  ...
  function mixColor(hex, amount, targetHex) {
    ...
    return (
      "#" +
      [mix(rgb.r, trg.r), mix(rgb.g, trg.g), mix(rgb.b, trg.b)]
...
#gpt-dom-trim-panel.dragging { cursor: grabbing; }

#gpt-dom-trim-main-row {
  display: flex;
  align-items: center;
  gap: 8px;
  min-width: 0;
...
/* ===== VERTICAL REFLOW MODE (fluid/constraint-based) ===== */
#gpt-dom-trim-wrapper.vertical #gpt-dom-trim-panel {
  width: 240px;
...
#gpt-dom-trim-wrapper.vertical #gpt-dom-trim-main-row {
  width: 100%;
  display: flex;
  flex-direction: column;
  align-items: stretch;
...
#gpt-dom-trim-wrapper.vertical #gpt-dom-trim-label {
  width: 100%;
  text-align: center;
  white-space: normal;
...
#gpt-dom-trim-wrapper.vertical #gpt-dom-trim-controls {
  width: 100%;
  display: flex;
  flex-wrap: wrap;
...
#gpt-dom-trim-wrapper.vertical #gpt-dom-trim-toggle { order: 1; margin: 0 auto; }
#gpt-dom-trim-wrapper.vertical #gpt-dom-trim-input-label {
  order: 2;
  flex: 0 0 100%;
  width: 100%;
  text-align: center;
...
#gpt-dom-trim-wrapper.vertical .gpt-dom-trim-btn { order: 3; }
#gpt-dom-trim-wrapper.vertical #gpt-dom-trim-input { order: 3; width: 56px; }

/* ===== Arrow (фикс размера, чтобы не раздувалась) ===== */
#gpt-dom-trim-arrow {
  position: absolute;
  left: 50%;
  bottom: -13px;
...
#gpt-dom-trim-arrow svg { width: 12px; height: 12px; transition: transform .18s ease; }
#gpt-dom-trim-wrapper.menu-open #gpt-dom-trim-arrow svg { transform: rotate(180deg); }
...
#gpt-dom-trim-wrapper.menu-open #gpt-dom-trim-menu { display: block; }

#gpt-dom-trim-wrapper.vertical #gpt-dom-trim-menu {
  min-width: 240px;
  max-width: min(90vw, 240px);
}
...
.gpt-dom-trim-menu-row:last-child { margin-bottom: 0; }

/* в vertical: строки меню reflow'ятся */
#gpt-dom-trim-wrapper.vertical .gpt-dom-trim-menu-row {
  display: flex;
  flex-wrap: wrap;
  align-items: center;
  justify-content: center;
...
/* заголовок пункта меню в vertical — отдельной строкой */
#gpt-dom-trim-wrapper.vertical .gpt-dom-trim-menu-title {
  flex: 0 0 100%;
  width: 100%;
  min-width: 0;
  text-align: center;
...
#gpt-dom-trim-opacity { width: 100%; min-width: 160px; flex: 1 1 auto; }
#gpt-dom-trim-wrapper.vertical #gpt-dom-trim-opacity {
  flex: 1 1 120px;
  min-width: 120px;
  width: auto;
}

#gpt-dom-trim-opacity-value { min-width: 44px; text-align: right; font-variant-numeric: tabular-nums; }
#gpt-dom-trim-wrapper.vertical #gpt-dom-trim-opacity-value {
  min-width: 44px;
  text-align: right;
}
...
.gpt-dom-trim-color-preset[data-color="#202123"] { background: #202123 !important; }
.gpt-dom-trim-color-preset[data-color="#343541"] { background: #343541 !important; }
.gpt-dom-trim-color-preset[data-color="#f2f2f2"] { background: #f2f2f2 !important; }
...
#gpt-dom-trim-toggle.on #gpt-dom-trim-toggle-knob { transform: translateX(20px); }
...
#gpt-dom-trim-rotate svg { width: 14px; height: 14px; flex: 0 0 auto; }
#gpt-dom-trim-wrapper.vertical #gpt-dom-trim-rotate .gpt-dom-trim-rotate-label {
  white-space: normal;
  overflow-wrap: anywhere;
}

/* ===== VERTICAL: “Цвет фона” — кнопки на отдельной строке (Повернуть слева, Пипетка справа) ===== */
#gpt-dom-trim-wrapper.vertical #gpt-dom-trim-rotate,
#gpt-dom-trim-wrapper.vertical #gpt-dom-trim-eyedropper {
...
/* Повернуть слева */
#gpt-dom-trim-wrapper.vertical #gpt-dom-trim-rotate { order: 100; }
/* Пипетка справа и уходит на нижнюю “линию” вместе с Повернуть */
#gpt-dom-trim-wrapper.vertical #gpt-dom-trim-eyedropper { order: 101; }

#gpt-dom-trim-eyedropper[disabled] { opacity: .5; cursor: not-allowed; }

.gpt-dom-trim-no-drag,
.gpt-dom-trim-no-drag * { cursor: auto; }
...
#gpt-dom-trim-menu label { user-select: none; }

#gpt-dom-trim-panel button,
#gpt-dom-trim-menu button,
#gpt-dom-trim-panel input,
#gpt-dom-trim-menu input { outline: none; }

#gpt-dom-trim-color-input::-webkit-color-swatch-wrapper { padding: 2px; }
#gpt-dom-trim-color-input::-webkit-color-swatch { border: none; border-radius: 4px; }
#gpt-dom-trim-color-input::-moz-color-swatch { border: none; border-radius: 4px; }
`;
  ...
  function applyPanelTheme() {
    ...
    const commonButtons = ui.wrapper.querySelectorAll(".gpt-dom-trim-btn, #gpt-dom-trim-eyedropper, #gpt-dom-trim-rotate");
    ...
    if (ui.toggle) {
      ...
      if (knob) knob.style.background = settings.enabled ? "#f8fafc" : (t.isLight ? "#ffffff" : "#f4f6fb");
    }
    ...
    labels.forEach((el) => (el.style.color = t.text));
  }
  ...
  function applyOrientationState() {
    if (!ui.wrapper) return;
    ui.wrapper.classList.toggle("vertical", settings.orientation === "vertical");
    if (ui.rotateBtn) {
      ui.rotateBtn.title = settings.orientation === "vertical"
        ? "Переключить в горизонтальное положение"
        : "Переключить в вертикальное положение";
    }
  ...
  function updateKeepInputUI() {
    if (ui.input) ui.input.value = String(settings.keepLast);
  }

  function updateOpacityUI() {
    if (ui.opacityRange) ui.opacityRange.value = String(settings.opacity);
    if (ui.opacityValue) ui.opacityValue.textContent = `${settings.opacity}%`;
    applyPanelTheme();
  }
  ...
  function ensurePositionDefaults() {
    ...
    if (!Number.isFinite(settings.panelY)) settings.panelY = 12;
  ...
  function createIconRotate() {
    return `
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
        <path d="M8 3H5v3"></path>
        <path d="M5 6a8 8 0 0 1 13.66-2.32L20 5"></path>
        <path d="M16 21h3v-3"></path>
        <path d="M19 18a8 8 0 0 1-13.66 2.32L4 19"></path>
      </svg>
    `;
  }
  ...
  function createControlPanel() {
    ...
    mainRow.id = "gpt-dom-trim-main-row";
    ...
    controls.id = "gpt-dom-trim-controls";
    ...
    rotateBtn.id = "gpt-dom-trim-rotate";
    rotateBtn.className = "gpt-dom-trim-no-drag";
    rotateBtn.type = "button";
    rotateBtn.innerHTML = `${createIconRotate()}<span class="gpt-dom-trim-rotate-label">Повернуть</span>`;
    ...
    rowColor.appendChild(rotateBtn);
    ...
    toggle.addEventListener("click", () => {
      ...
      saveSettings();
      updateToggleUI();
      if (settings.enabled) scheduleTrim();
      else restoreTrimmedMessages();
    });

    ui.presetButtons.forEach((btn) => btn.addEventListener("click", () => setBackgroundColor(btn.dataset.color)));

    const btnDecEl = btnDec;
    const btnIncEl = btnInc;

    btnDecEl.addEventListener("click", () => applyKeep(settings.keepLast - 1));
    btnIncEl.addEventListener("click", () => applyKeep(settings.keepLast + 1));
    ...
    colorInput.addEventListener("input", () => setBackgroundColor(colorInput.value));
    ...
    eyeDropperBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (result && result.sRGBHex) setBackgroundColor(result.sRGBHex);
      } catch (err) {
  ...
  function getAllTurns() {
    ...
    if (!turns || !turns.length) turns = root.querySelectorAll(`[data-message-author-role], .${STUB_CLASS}`);
  ...
  function restoreStub(stub) {
    ...
    if (restored) stub.replaceWith(restored);
  }
  ...
  function initObserver() {
    turnsContainer = document.querySelector('[data-testid="conversation-turns"]') || document.querySelector("main");
    ...
    observer.observe(turnsContainer, { childList: true, subtree: true });
    scheduleTrim();
  }
  ...
  function initResizeHandler() {
    window.addEventListener("resize", () => clampAndApplyPosition(false));
  }
